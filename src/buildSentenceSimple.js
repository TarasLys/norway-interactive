...
function displaySentence() {
  ...
  initialWordCount = wordsContainer.children.length; // Запоминаем начальное количество слов
}
...
function removeLastWord() {
  if (selectedWords.length > 0) {
    ...
    if (!isVoiceInput && document.getElementById("wordsContainer").children.length < initialWordCount) {
      const wordElement = document.createElement("button");
