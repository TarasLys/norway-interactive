...
function selectWord(word, wordElement) {
  ...
  const selectedSentence = selectedWords.filter(w => w !== undefined && w !== "").join(" ").replace(/\s*([,\.!?])\s*/g, "$1 ");
...
function formatSentence(sentence, originalSentence) {
  if (typeof sentence === 'undefined') {
    return '';
  }

  sentence = sentence.charAt(0).toUpperCase() + sentence.slice(1);
  sentence = sentence.replace(/\b\d+\b/g, match => numberMap[match] || match);

  originalSentence.split('').forEach((char, i) => {
    if (['.', ',', '!', '?'].includes(char)) {
      sentence = sentence.slice(0, i) + char + (sentence[i + 1] === ' ' ? '' : ' ') + sentence.slice(i + 1);
    }
  });
...
function capitalize(text) {
  ...
  const exceptions = ["og", "men", "eller", "fordi", "hvis", "når", "hvorfor", "hvordan", "hva", "hvem", "derfor", "som", "at", "om", "så", "på", "i", "til", "med", "av", "fra", "for", "over", "under", "eller", "nå", "her", "der", "hvor", "når", "da", "skal", "kan", "vil", "må", "gå", "spise", "drikke", "se", "høre", "være", "gjøre", "ta", "ha", "få", "sove", "stå", "sitte", "ligge", "leke", "arbeide", "spille", "kjenne"];

  return words.map((word, index) => {
    if (index === 0 || /^[A-Z][a-z]*$/.test(word) || (/^[A-Z][a-z]*$/.test(word) && !exceptions.includes(word.toLowerCase()))) {
...
function checkAnswer() {
  const selectedSentence = normalizeSentence(selectedWords.filter(w => w !== undefined && w !== "").join(" "));
  const correctSentence = normalizeSentence(currentSentence.norwegian);
  const formattedSelectedSentence = capitalize(formatSentence(selectedSentence, currentSentence.norwegian));
  const formattedCorrectSentence = capitalize(formatSentence(correctSentence, currentSentence.norwegian));
  ...
  if (formattedSelectedSentence === formattedCorrectSentence) {
    ...
    document.getElementById("correctAnswer").textContent = `Riktig svar: ${formattedCorrectSentence.replace(/,,+/g, ',').replace(/\.\.+/g, '.').replace(/\?\?+/g, '?')}`;
  ...
  selectedWords = [];
  displaySentence();
  speak(formattedCorrectSentence.replace(/,,+/g, ',').replace(/\.\.+/g, '.').replace(/\?\?+/g, '?'));
}
...
function startVoiceInput() {
  if (!recognition) {
    ...
    recognition.onresult = function(event) {
      ...
      transcript = transcript.replace(/:00/g, '');
      ...
      selectedWords = words;
      const selectedSentence = selectedWords.filter(w => w !== undefined && w !== "").join(" ").replace(/\s*([,\.!?])\s*/g, "$1 ");
      const formattedSentence = capitalize(formatSentence(selectedSentence, currentSentence.norwegian));

      let finalSentence = formattedSentence.replace(/\.\.+/g, '.').replace(/,,+/g, ',').replace(/\?\?+/g, '?');

      document.getElementById("sentence").textContent = finalSentence;
      document.getElementById("feedback").textContent = "";
      document.getElementById("correctAnswer").textContent = "";
      document.getElementById("userAnswer").textContent = "";
    };
...
//   console.log("Selected Sentence:", formattedSelectedSentence);
//   console.log("Correct Sentence:", formattedCorrectSentence);
...
//     // Убираем ":00" из пользовательского ответа
//     const filteredSelectedSentence = formattedSelectedSentence.replace(/:00/g, '');
//     document.getElementById("userAnswer").textContent = `Din svar: ${filteredSelectedSentence}`;
...
//       // Удаляем "undefined" из списка слов и время в формате ":00"
//       words = words.filter(word => word.toLowerCase() !== "undefined" && word !== ":00");
